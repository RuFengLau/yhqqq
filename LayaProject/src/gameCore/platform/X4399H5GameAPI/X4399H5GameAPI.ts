...
export default class X4399H5GameAPI implements PlatformsAPI {
    ...
    onInit(_callback: Function) {
        console.log('X4399H5GameAPI onInit');
    }
    /**登录 */
    onLogin() {
        console.log('X4399H5GameAPI onLogin');
    }
    /**分享 */
    onShare() {
        console.log('X4399H5GameAPI onShare');
    }
    /**上传最高分(排行榜) */
    updateHighScore(_score: number) {
        console.log('X4399H5GameAPI updateHighScore');
    }
    /**初始化Ad (部分渠道需要先初始化广告 如oppo)*/
    onInitAdService() {
        console.log('X4399H5GameAPI onInitAdService');
    }
    /**创建广告 BannerAd */
    createBannerAd() {
        console.log('X4399H5GameAPI createBannerAd');
    }
    /**展示Banner广告 */
    onShowBanner() {
        console.log('X4399H5GameAPI onShowBanner');
    }
    /**隐藏Banner广告 */
    onHideBanner() {
        console.log('X4399H5GameAPI onHideBanner');
    }
    /**创建激励视频广告 RewardedVideoAd */
    createRewardedVideoAd() {
        console.log('X4399H5GameAPI createRewardedVideoAd');
    }
    /**展示RewardedVideo激励视频广告 */
    onShowRewardedVideo(_successCallback: Function, _failCallback: Function) {
        console.log('X4399H5GameAPI onShowRewardedVideo');
    }
    /**创建插屏广告 createInsertAd */
    createInsertAd() {
        console.log('X4399H5GameAPI createInsertAd');
    }
    /**展示InsertAd插屏广告 */
    onShowInsertAd() {
        console.log('X4399H5GameAPI onShowInsertAd');
    }
    /**创建原生广告 createNativeAd */
    createNativeAd() {
        console.log('X4399H5GameAPI createNativeAd');
    }
    onShowNativeAd() {
        console.log('X4399H5GameAPI onShowNativeAd');
    }
    onNativeAdClick(_id: string) {
        console.log('X4399H5GameAPI onNativeAdClick:' + _id);
    }
    /**存储数据
    ...
    saveDataToCache(_key: string, _value: any) {
        ...
        console.log('X4399H5GameAPI saveDataToCache');
    }
    /**读取数据
     * @param _key 索引 string
     */
    readDataFromCache(_key: string) {
        console.log('X4399H5GameAPI readDataFromCache');
