 ...
 * 摇杆类-FGUI
 * 用法
 * this._joystick = new Joystick(this.view);
 * this._joystick.on(Joystick.JoystickMoving, this, this.onJoystickMoving);
 * this._joystick.on(Joystick.JoystickUp, this, this.onJoystickUp);
 * 
 * onJoystickMoving 参数 angle
 */

export default class Joystick extends Laya.EventDispatcher {
    ...
    private OnTouchUp(evt: Laya.Event): void {
        if (this.touchId != -1 && evt.touchId == this.touchId) {
            ...
            Laya.stage.off(Laya.Event.MOUSE_UP, this, this.OnTouchUp);
            Laya.stage.off(Laya.Event.MOUSE_OUT, this, this.OnTouchUp);
            this.event(Joystick.JoystickUp);
        }
    }
    ...
    private OnTouchMove(evt: Laya.Event): void {
        if (this.touchId != -1 && evt.touchId == this.touchId) {
            ...
            this.event(Joystick.JoystickMoving, degree);
        }
    }
}